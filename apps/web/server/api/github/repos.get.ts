  ...
  return withD1Cache(
    ...
    async () => {
      try {
        ...
        const reposRes = await $fetch<GithubRawRepo[]>(
          'https://api.github.com/orgs/narduk-enterprises/repos?sort=updated&per_page=100',
          {
            headers,
          },
        )
        ...
        const reposWithActions = await Promise.all(
          ...
          repos.map(async (repo: GithubRawRepo) => {
            ...
            try {
              const runsRes = await $fetch<GithubRawRuns>(
