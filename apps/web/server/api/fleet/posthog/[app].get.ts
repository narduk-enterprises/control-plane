...
export default defineEventHandler(async (event) => {
  ...
  let start = queryParams.startDate ? new Date(queryParams.startDate) : new Date(end.getTime()) // Default: today only
  ...
  `
      ...
      try {
        ...
        const checkRes = async <T>(res: Response, name: string): Promise<T> => {
          ...
          if (import.meta.dev) {
            ...
            console.log(
              `[PostHog Response] ${name}: ${(d.results as unknown[])?.length ?? 0} rows, columns: ${JSON.stringify(d.columns ?? [])}`,
            )
          }
          return data
        }
        ...
        if (isSummaryOnly) {
          ...
          const summaryCols = summaryData.columns ?? [
            'event_count',
            'unique_users',
            'pageviews',
            'sessions',
          ]
          const summary = summaryCols.reduce(
            (acc: Record<string, number>, col: string, idx: number) => {
      ...
      `
          ...
          const timeSeries = (timeSeriesData.results ?? []).map((row) => ({
            date: String(row[0]),
          ...
          const topPages = batchedRows
            .filter((r) => r[0] === 'page')
            .map((r) => ({ name: String(r[1] || '/'), count: Number(r[2] ?? 0) }))
          const topReferrers = batchedRows
            .filter((r) => r[0] === 'referrer')
            .map((r) => ({ name: String(r[1] || 'Direct'), count: Number(r[2] ?? 0) }))
          const topCountries = batchedRows
            .filter((r) => r[0] === 'country')
            .map((r) => ({ name: String(r[1] || 'Unknown'), count: Number(r[2] ?? 0) }))
          const topBrowsers = batchedRows
            .filter((r) => r[0] === 'browser')
            .map((r) => ({ name: String(r[1] || 'Unknown'), count: Number(r[2] ?? 0) }))

          const uiHost = host.includes('us.i.posthog.com')
            ? 'https://us.posthog.com'
            : 'https://eu.posthog.com'
          const replaysUrl = `${uiHost}/project/${projectId}/replays?properties=[{"key":"$host","value":["${encodeURIComponent(appHost)}"],"operator":"exact","type":"event"}]`
