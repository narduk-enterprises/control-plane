...
const badgeConfig = computed(() => {
  ...
  if (props.appStatus.status === 'down') {
    return {
      ...
      label: 'Down',
      class: 'ring-1 ring-error/50 font-medium',
    }
  }
...
const isRefreshing = computed(() =>
  props.appStatus?.app ? isAppRefreshing(props.appStatus.app) : false,
)
...
<template>
  <div class="flex items-center gap-1.5">
    <UTooltip :text="checkedAgo || 'Not checked yet'">
      <UBadge :color="badgeConfig.color" variant="subtle" size="sm" :class="badgeConfig.class">
    ...
    <UButton
      ...
      :class="[
        ...
        isMounted &&
          isRefreshing &&
          'animate-spin text-primary hover:text-primary relative scale-110',
      ]"
