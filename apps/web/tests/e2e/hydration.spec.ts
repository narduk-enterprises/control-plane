...
test.describe('SSR Hydration Safety Checks', () => {
    ...
    for (const route of routesToCheck) {
        test(`Check hydration on ${route}`, async ({ page }) => {
            ...
            page.on('console', msg => {
                const text = msg.text()
                if (text.includes('Hydration') || text.includes('Hydration class mismatch') || text.includes('Hydration children mismatch')) {
            ...
            expect(hydrationErrors, `Hydration mismatches found on ${route}: \n${hydrationErrors.join('\n')}`).toEqual([])
        })
    }
})
