...
function loadCredentials(): Record<string, any> {
    ...
    throw new Error(
        'No service account credentials found. Set GSC_SERVICE_ACCOUNT_JSON in Doppler.'
    )
}
...
async function main() {
    ...
    if (!siteUrlArg) {
        ...
        console.error('Usage: doppler run -p control-plane -c prd -- npx tsx tools/grant-gsc-access.ts <site_url>')
        process.exit(1)
    }
    ...
    try {
        ...
        if (message.includes('not verified') || status === 400 || message.includes('We couldn\'t find your verification file')) {
            console.error(`\n❌ Failed to verify ownership automatically.`)
            console.error(`This usually means the verification file (e.g., googleXXXX.html) is not deployed to the live site at ${siteUrl}.`)
            console.error(`To fix this: Run 'npx jiti tools/setup-analytics.ts gsc' inside the app's repo, deploy it, and try this script again.`)
            console.error(`\nRaw error: ${message}`)
        } else if (status === 403) {
            console.error(`\n❌ 403 Forbidden.`)
            console.error(`The service account does not have permission to access the Webmaster API or the property isn't verified.`)
