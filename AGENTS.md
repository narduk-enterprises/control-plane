...
3. In `nuxt.config.ts`, declare all secrets in `runtimeConfig` with explicit
   ...
   ```
4. **Important:** Most Doppler env var names are the **raw key names** (e.g.
   `POSTHOG_PUBLIC_KEY`, `SITE_URL`) and are read directly via `process.env.KEY`
   at build time. The intentional Nuxt-prefixed exceptions in this stack are
   `NUXT_SESSION_PASSWORD` and `NUXT_PORT`.
...
3. Create `playwright.config.ts`:
   ...
   const baseURL = `http://localhost:${Number.isFinite(port) ? port : 3000}`
